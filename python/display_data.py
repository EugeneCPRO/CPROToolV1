...
def fetch_wallet_balances(collection):
    try:
        ...
        if wallet_data:
            ...
            portfolio_data = list({'asset': asset, 'balance': details.get('confirmedBalance', 'Unknown'), 'value': details.get('value', 'Unknown')} for asset, details in data.items())
...
def update_tree_values(tree, collection, total_value_label):
    ...
    while True:
        ...
        if wallet_data:
            ...
            new_sorted_data = sorted(wallet_data, key=lambda item: item['value'], reverse=True)
            ...
            if new_sorted_data != sorted_data:
                ...
                for details in new_sorted_data:
                    asset = details['asset']
                    confirmed_balance = details.get('balance', 'Unknown')
                    current_value = float(details.get('value', 'Unknown'))

                    # Get the item ID based on the asset or create a new one
                    item_id = tree.insert("", "end", values=(details["asset"], round(float(confirmed_balance), 5), format_value(current_value)))
